...
const VERDICT_COLORS: Record<ActionVerdict, { badge: string; border: string }> =
  {
    correct: {
      badge: "badge--nominal",
      border: "border-l-[var(--color-nominal)]",
    },
    partial: {
      badge: "badge--caution",
      border: "border-l-[var(--color-caution)]",
    },
    incorrect: {
...
export function EventCard({
  ...
  const actorLabel =
    event.actor.actor_type.charAt(0).toUpperCase() +
    event.actor.actor_type.slice(1);
  const annotatable = isAnnotatable(event);

  const displayAnnotation = annotation ?? autoAnnotation;
  const verdictStyle = displayAnnotation
    ? VERDICT_COLORS[displayAnnotation.verdict]
    : null;
  ...
  const handleVerdictChange = (verdict: ActionVerdict) => {
    ...
    onAnnotationChange({
      ...
      verdict,
      should_have_done:
        annotation?.should_have_done ?? autoAnnotation?.should_have_done ?? "",
      reasoning: annotation?.reasoning ?? autoAnnotation?.reasoning ?? "",
    });
  };

  const handleFieldChange = (
    field: "should_have_done" | "reasoning",
    value: string
  ) => {
    if (!onAnnotationChange) return;
    onAnnotationChange({
  ...
  return (
    <div className="relative flex gap-3">
      ...
      <div className="flex flex-col items-center w-6 shrink-0">
        <div
          className={`w-px flex-1 ${isFirst ? "bg-transparent" : "bg-border-default"}`}
        />
        <div
          className={`w-2.5 h-2.5 rounded-full shrink-0 ${colors.dot} shadow-sm`}
        />
        <div
          className={`w-px flex-1 ${isLast ? "bg-transparent" : "bg-border-default"}`}
        />
      </div>

      {/* Content */}
      <div
        className={`flex-1 pb-4 ${annotatable && verdictStyle ? `pl-2.5 border-l-2 ${verdictStyle.border}` : ""}`}
      >
        {/* Header */}
        <div className="flex items-center gap-2 mb-1">
          ...
          <span
            className={`font-mono text-[10px] font-medium uppercase tracking-wider ${colors.text}`}
          >
            {actorLabel}
          </span>
          {event.actor.name && (
        ...
        {event.message && (
          <div
            className={`${colors.bg} border border-border-dim rounded-sm px-3 py-2`}
          >
          ...
          <div className="mt-2">
            <div className="flex items-center gap-2">
              <span className={`badge ${verdictStyle?.badge} text-[9px]`}>
                {autoAnnotation?.verdict === displayAnnotation.verdict
                  ? "AI: "
                  : ""}
                {ACTION_VERDICTS.find(
                  (v) => v.value === displayAnnotation.verdict
                )?.label ?? displayAnnotation.verdict}
              </span>

              {/* Violation count badges */}
              {instrViolations.length > 0 && (
                <span className="badge badge--critical text-[9px] py-0.5 px-1.5">
                  {instrViolations.length} rule
                  {instrViolations.length > 1 ? "s" : ""} violated
                </span>
              )}
              {ctxViolations.length > 0 && (
                <span className="badge badge--caution text-[9px] py-0.5 px-1.5">
                  {ctxViolations.length} context issue
                  {ctxViolations.length > 1 ? "s" : ""}
                </span>
              )}
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      {cv.expected !== undefined && (
                        <p className="text-tertiary mt-0.5">
                          Expected: {JSON.stringify(cv.expected)} — Got:{" "}
                          {JSON.stringify(cv.actual)}
                        </p>
                      )}
                    </div>
                  </div>
              ...
              <div className="mt-2 p-3 bg-void border border-border-dim rounded-sm space-y-2.5">
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {ACTION_VERDICTS.map((v) => (
                      <button
                        key={v.value}
                        onClick={() =>
                          handleVerdictChange(v.value as ActionVerdict)
                        }
                        className={`badge text-[10px] cursor-pointer transition-all ${
                          (annotation?.verdict ?? autoAnnotation?.verdict) ===
                          v.value
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    placeholder={
                      autoAnnotation?.should_have_done ||
                      "Describe the ideal action..."
                    }
                    value={annotation?.should_have_done ?? ""}
                    onChange={(e) =>
                      handleFieldChange("should_have_done", e.target.value)
                    }
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    placeholder={
                      autoAnnotation?.reasoning || "Why this verdict?"
                    }
                    value={annotation?.reasoning ?? ""}
                    onChange={(e) =>
                      handleFieldChange("reasoning", e.target.value)
                    }
                  />
                </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {instrViolations.map((iv, i) => (
                        <div
                          key={i}
                          className="bg-critical-bg border border-border-dim rounded-sm px-2.5 py-1.5"
                        >
                          <div className="flex items-center gap-2">
                            <span className="font-mono text-[10px] text-critical font-medium">
                              {iv.instruction_id}
                            </span>
                            <span className="text-[10px] text-tertiary italic truncate">
                              {iv.instruction_text}
                            </span>
                          </div>
                          <p className="text-[11px] text-critical mt-0.5">
                            {iv.violation_description}
                          </p>
                          {iv.context_evidence && (
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {ctxViolations.map((cv, i) => (
                        <div
                          key={i}
                          className={`border border-border-dim rounded-sm px-2.5 py-1.5 ${
                            cv.severity === "critical"
                              ? "bg-critical-bg"
                              : "bg-caution-bg"
                          }`}
