...
const getActorDisplay = (event: EventEnvelope): string => {
  ...
  if (typeof actor?.name === "string" && actor.name.length > 0)
    return actor.name;
  if (typeof actor?.actor_id === "string" && actor.actor_id.length > 0)
...
export function EventsClient({
  ...
  const [timelinePlayback, setTimelinePlayback] = useState<
    "live" | "playing" | "paused"
  >("paused");
  ...
  const [recordingState, setRecordingState] =
    useState<RecordingState>(REC_IDLE);
  ...
  const [messengerStep, setMessengerStep] = useState<"provider" | "appId">(
    "provider"
  );
  const [messengerProvider, setMessengerProvider] = useState<string | null>(
    null
  );
  ...
  const fetchEvents = useCallback(
    async (options?: { silent?: boolean }) => {
      try {
        ...
        setAvailableSources(
          Array.from(
            new Set((data || []).map((event) => event.event.source))
          ).sort()
        );
      } catch (err) {
  ...
  useEffect(() => {
    if (
      timelineFilter !== "all" &&
      !availableSources.includes(timelineFilter)
    ) {
  ...
  useEffect(() => {
    if (
      prevRecordingKindRef.current !== "Recording" &&
  ...
  const unloadMessengerWidget = useCallback(() => {
    ...
    document
      .querySelectorAll('script[src^="https://widget.intercom.io/widget/"]')
      .forEach((el) => el.remove());
  }, []);

  const loadMessengerWidget = useCallback(
    (appId: string) => {
      unloadMessengerWidget();
      (
        window as Window & { intercomSettings?: Record<string, unknown> }
      ).intercomSettings = {
        app_id: appId,
      ...
      script.onload = () => {
        ...
        localStorage.setItem(
          ...
          JSON.stringify({ provider: "intercom", appId })
        );
      };
      script.onerror = () => {
        setMessengerStatus("error");
        setMessengerError(
          "Could not load the messenger. Check the App ID and try again."
        );
      };
  ...
  useEffect(() => {
    ...
    if (
      stored?.provider === "intercom" &&
      typeof stored?.appId === "string" &&
      stored.appId.trim()
    ) {
              ...
              {messengerStep === "provider" ? (
                <>
                  <p className="text-xs text-tertiary">
                    Choose a provider to open its messenger and send test
                  ...
                  >
                    <span className="w-8 h-8 bg-data-bg border border-data flex items-center justify-center text-data font-mono text-xs font-bold">
                      IN
                    </span>
                ...
                <>
                  {messengerStatus === "loaded" ? (
                    ...
                    <>
                      <p className="text-xs text-tertiary">
                        Enter your Intercom workspace App ID. Load the messenger
                        to send test events.
                      </p>
                      <label
                        htmlFor="events-messenger-app-id"
                        className="block text-xs text-tertiary uppercase tracking-wide mb-1"
                      >
                        App ID
                      </label>
                      <input
                      ...
                      {messengerStatus === "error" && messengerError && (
    ...
    <div className="flex gap-1 border-b border-border-default mb-4">
      <button
        ...
        className={`px-4 py-2 text-lg font-medium transition-colors ${
          viewTab === "list"
            ? "text-data border-b-2 border-data bg-transparent"
            ...
            <div className="panel__content">
              <div className="flex flex-col items-center justify-center py-12 text-center">
                <div className="w-16 h-16 border border-caution-dim bg-caution-bg flex items-center justify-center mb-4">
                  <svg
                    className="w-8 h-8 text-caution"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                    strokeWidth={1.5}
                ...
                <div className="text-base font-medium text-primary mb-2">
                  Unable to connect to Events Manager
                </div>
            ...
            <div className="panel__content">
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <div className="text-sm text-tertiary mb-6 max-w-sm">
                  Events will appear here once data sources begin transmitting.
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`badge ${getActorBadge(getActorType(event))}`}
                          >
                            {getActorType(event)}
                          </span>
                        </div>
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          <span className="font-mono text-xs text-tertiary tabular-nums">
                            {formatTime(event.event.event_time)}
                          </span>
                        </div>
                      </div>
                  ...
                  {selectedEvent ? (
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-data font-medium">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <span
                            className={`badge ${getActorBadge(getActorType(selectedEvent))}`}
                          >
                            {getActorType(selectedEvent)}
                          </span>
                          <span className="text-sm text-secondary">
                            {getActorDisplay(selectedEvent)}
                          </span>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="font-mono text-xs text-secondary break-all">
                          {getPrimaryEntity(selectedEvent)}
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="font-mono text-sm text-primary tabular-nums">
                          {new Date(
                            selectedEvent.event.event_time
                          ).toLocaleString()}
                        </div>
                      </div>
