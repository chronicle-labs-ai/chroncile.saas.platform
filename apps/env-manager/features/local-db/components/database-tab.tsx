...
import type { ContainerStatus, MigrationStatus, DbInfo } from "@/lib/local-db";
...
export function DatabaseTab() {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="panel__content bg-[var(--critical-bg)] space-y-3">
            ...
            <div className="text-xs text-secondary space-y-1">
              ...
              <ol className="list-decimal list-inside space-y-1 text-secondary">
                <li>
                  Open{" "}
                  <span className="font-mono text-primary">Docker Desktop</span>{" "}
                  from Applications, or run{" "}
                  <code className="font-mono text-data bg-[var(--bg-hover)] px-1.5 py-0.5 rounded">
                    open -a Docker
                  </code>
                </li>
                <li>
                  Wait for the Docker engine to start (whale icon in menu bar
                  stops animating)
                </li>
                <li>
                  This page will automatically detect Docker once it is running
                </li>
              </ol>
            </div>
          </div>
      ...
      <SeedsPanel pgReady={data.container.pgReady} onRefresh={onRefresh} />
