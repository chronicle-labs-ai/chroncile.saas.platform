...
export function BackendPanel({
  ...
  const doRestart = async () => {
    ...
    setShowLogs(true);
    try {
      const res = await fetch("/api/local-db/restart-backend", { method: "POST" });
    ...
    <div className="panel">
      <div className="panel__header">
        <div className="flex items-center gap-2">
          <span className={`status-dot ${backendHealthy ? "status-dot--nominal" : "status-dot--critical"}`} />
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-[10px] text-tertiary uppercase tracking-wider">
            {backendHealthy ? "healthy" : backendPid ? "unhealthy" : "not running"}
          </span>
        </div>
      </div>
      <div className="panel__content space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <div>
            ...
            <span className="font-mono text-sm text-primary">{backendPid ?? "—"}</span>
          </div>
          <div>
            <span className="label block mb-1">Health</span>
            <span className={`font-mono text-sm ${backendHealthy ? "text-nominal" : "text-critical"}`}>
        ...
        <div className="flex items-center gap-3 pt-3 border-t border-border-dim">
          ...
          {restartState === "success" && (
            <span className="text-xs text-nominal font-mono">Restarted successfully</span>
          )}
          {restartState === "error" && (
            <span className="text-xs text-critical font-mono">{restartError ?? "Restart failed"}</span>
          )}
        </div>
          ...
          <div className="pt-3 border-t border-border-dim">
            ...
            <div className="max-h-80 overflow-y-auto bg-[var(--bg-sunken)] rounded">
              <pre className="px-3 py-2 text-[11px] font-mono text-secondary whitespace-pre-wrap break-all leading-relaxed">
                {logsData?.logs || "No logs yet. Restart the backend to see output here."}
              </pre>
            </div>
          </div>
        )}
