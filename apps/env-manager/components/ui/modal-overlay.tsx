...
export function ModalOverlay({ onClose, maxWidth = "max-w-md", children }: ModalOverlayProps) {
  return (
    <div
    ...
    >
      ...
      <div className={`relative w-full ${maxWidth} mx-4 panel`}>
        {children}
      </div>
    </div>
  );
    ...
    <>
      ...
      {emailError && (
        <div className="flex items-center gap-2 p-2 bg-caution-bg border border-caution-dim rounded-sm">
          ...
          <span className="text-xs text-caution">Email failed: {emailError}</span>
        </div>
      )}
    </>
  );
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 bg-elevated border border-border-default rounded-sm px-3 py-2">
        <span className="font-mono text-xs text-data flex-1 truncate">{loginUrl}</span>
        <button
