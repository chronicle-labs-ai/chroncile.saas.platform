...
export function FilterPill({
  ...
  return (
    <div
      ...
      className={cx(
        "inline-flex items-stretch h-[22px] text-[11.5px] text-l-ink select-none overflow-hidden",
        "rounded-[4px] border border-l-border bg-l-wash-2",
        "[&>span+span]:border-l [&>span+span]:border-l-border",
        className,
      )}
      {...props}
    >
      <span
        className={cx(baseSegment, "text-l-ink-lo hover:bg-l-wash-3 hover:text-l-ink")}
        onClick={onEditDim}
      >
      ...
      <span
        className={cx(
          "inline-flex items-center px-[6px] text-l-ink-dim font-mono text-[10.5px] cursor-default",
        )}
      >
        {verb}
      </span>
      ...
      {onRemove ? (
        <span
          ...
          className={cx(
            "inline-flex items-center px-[5px] text-l-ink-dim cursor-pointer",
            "hover:bg-[rgba(239,68,68,0.12)] hover:text-event-red",
          )}
          onClick={(e) => {
            e.stopPropagation();
            onRemove();
