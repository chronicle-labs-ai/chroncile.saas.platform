...
/**
 * useTimeView — pan / zoom state for the timeline.
 *
 * Models the visible time window as `(centerMs, halfWidthMs)` because
 * zooming around a point is much cleaner with a center than with a
 * `[start, end]` pair. Returns helpers for converting back and forth
 * to pixel coordinates given the time-area width.
 */
...
export function useTimeView(
  ...
  const viewRef = useRef({ centerMs, halfWidthMs });
  useEffect(() => {
  ...
  const expandToInclude = useCallback((timeMs: number, marginRatio = 0.1) => {
    ...
    if (timeMs < start) {
      ...
      setHalfWidthMs((prev) =>
        Math.min(MAX_HALF_WIDTH_MS, Math.max(MIN_HALF_WIDTH_MS, prev + delta / 2)),
      );
    } else if (timeMs > end) {
  ...
  const fitToTimes = useCallback(
    (timesMs: readonly number[], paddingRatio = 0.1) => {
...
export type UseTimeViewReturn = ReturnType<typeof useTimeView>;
