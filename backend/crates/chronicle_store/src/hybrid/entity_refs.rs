//! `EntityRefStore` for the hybrid backend -- delegates entirely to Postgres.
//!
//! Entity refs always live in Postgres (they are small, relational, and
//! need ACID for JIT linking). They reference event_ids that may live
//! in either Postgres (hot) or Parquet (cold).
...
#[async_trait]
impl EntityRefStore for HybridBackend {
